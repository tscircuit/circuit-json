...
test("source_pin_must_be_connected_error without subcircuit_id", () => {
  const validError = {
    type: "source_pin_must_be_connected_error" as const,
    message: "Port VCC on U1 must be connected but is floating",
  ...
  expect(result.message).toBe(
    "Port VCC on U1 must be connected but is floating",
  )
  expect(result.source_component_id).toBe("source_component_1")
  expect(result.source_port_id).toBe("source_port_1")
