...
test("schematic_symbol accepts optional metadata.kicad_symbol", () => {
  const symbol = schematic_symbol.parse({
    type: "schematic_symbol",
  ...
  expect(symbol.metadata?.kicad_symbol?.symbolName).toBe(
    "Amplifier_Operational:AD8603",
  )
  expect(symbol.metadata?.kicad_symbol?.pinNames?.offset).toBe(1.27)
  expect(symbol.metadata?.source).toBe("kicad")
})
